# 📌 Pinterest Clone

Une reproduction moderne de l’interface Pinterest en version Web & Mobile, développée avec **HTML, CSS et JavaScript** sur VSCode.

## ✨ À propos du projet

Ce projet a été réalisé dans le but de progresser en :

- Front-end Development
- UI/UX Design
- Responsive Design
- Création d’interfaces inspirées des applications modernes

## 🎨 Fonctionnalités

- Interface inspirée de Pinterest
- Version Web & Mobile responsive
- Design naturel et moderne
- Animations et interactions personnalisées
- Expérience utilisateur fluide

## 📸 Ressources utilisées

- Photos prises par moi-même
- Icônes modifiées/créées afin de respecter les droits d’auteur

## 🚀 Démo en ligne

🌐 Démo du site :  
[Pinterest Clone – Live Demo](https://arsellor.github.io/Pinterest_Clone/?utm_source=chatgpt.com)]

## 💻 Code source

🔗 GitHub Repository :  
[Pinterest_Clone GitHub Repository](https://github.com/Arsellor/Pinterest_Clone.git?utm_source=chatgpt.com)]

## 📱 Réseaux & Vidéos

🎥 YouTube Short :  
[Voir la vidéo YouTube](https://youtube.com/shorts/9H8-hVAcfWQ?si=Oy5VXEHEhROevakR&utm_source=chatgpt.com).]

🎵 TikTok :  
[Voir la vidéo TikTok](https://www.tiktok.com/@tantchi/video/7639156916073073942?is_from_webapp=1&sender_device=pc&utm_source=chatgpt.com)
⭐ N’hésitez pas à laisser une étoile sur le projet si vous aimez le concept.

